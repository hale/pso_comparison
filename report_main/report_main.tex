...
%\begin{description}
  %\item[Type] local-best, with adaptive-random topology.
  %\item[Particles] 30
  %\item[Number of neighbourhoods]
  %\item[Initial velocity] idk
  %\item[Inertia] $1/(2*log(2))$
  %\item[$c_1$] $0.5 * log(2)$
  %\item[$c_2$] $0.5 * log(2)$
%\end{description}
...
Canonical PSO groups neighbours in a ring topology, SPSO-2011 implements
adaptive-random topology. This introduces variability into the neighbourhoods,
whereby the neighbourhood changes each time the local-best score stays the same.
...
\begin{Figure}
  \centering
  \includegraphics[width=0.5\linewidth]{star_topology.eps}
...
The following parameters were setup for both simulations:

\begin{description}
  \item[Swarm size] 40
  \item[Maximum function evaluations] 100000
