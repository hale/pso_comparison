...
The question we are asking is: to what extent has PSO imrpoved in the past 10
years?
...
Comparing the contemporary baseline-performance of PSO with early PSO allows us
to assess how far the field has come, and to what extent some of PSO's earlier
limitations are still relevant.
...
\begin{enumerate}
  ...
  \item{A contemporary PSO cannot converge closer to the global minimum
      than an original PSO in a given number of function evaluations.}
\end{enumerate}

These hypotheses are predicted outcomes of the experiments:

\begin{enumerate}
  \item{A contemporary PSO can converge closer to the
      global minimum than an original PSO in a given number of function evaluations.}
  \item{A contemporary PSO can converge more quickly to the global minimum than an
      original PSO.}
\end{enumerate}

\section{Experimental Design}

In order to test the hypotheses, two PSO implementations were selected. One to
model an early PSO variant, and another to model the current gold-standard of
PSO research.
...
velocity and positional clamping.
...
Three objective functions widely used in optimization literature have been tested
against, the results of which appear below. Experiments have been carried out
first in 2 and then 20 dimensions.

A Sphere is the simplest of the chosen optimization targets, since there are no local
minimum `traps' that can result in premature convergence. The Rosenbrock
function is a widely used test function in optimization. PSO was first tested against
...
% STATE THE OBVIOUS; STATEMENTS ON RESULTS
